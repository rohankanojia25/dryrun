...
function renderTradeCol(){
  ...
  el.innerHTML = `
    ...
    <div class="side">
      <h3 style="color:var(--brand2)">LIVE TRADES</h3>
      <div id="tape"><div class="hint" style="margin:0">Waiting for trades…</div></div>
    </div>`;
  renderTape();
}
function renderTape(){
  ...
  el.innerHTML = tape.slice(0,20).map(x=>`
    <div class="hist"><span class="${x.side==='buy'?'b':'s'}">${x.side.toUpperCase()}</span> ${fmt(x.sol,3)} SOL · <span class="mono">${money(x.mc)} MC</span> · ${x.t}</div>`).join('');
}
function renderPage(){
  const t = tk();
...
(async function(){
  ...
  known[MINT] = known[MINT] || { mint:MINT, symbol:Q.get('sym')||'…', name:'', pair:Q.get('pair')||'', priceUsd:0, priceSol:0, dex:'pump-fun' };
  await ensureSolUsd();
  renderPage();
  try{
  ...
  // live tick-by-tick stream for this token
  wsConnect();
  wsSubTrades([MINT], d=>{
    if(d.mint!==MINT) return;
    const t = ppApply(d); if(!t) return;
    const map={}; map[MINT]=t;
    flashCells(map);
    renderTokenStats();
    tape.unshift({side:d.txType, sol:+d.solAmount||0, mc:t.mc||0, t:new Date().toLocaleTimeString()});
    tape = tape.slice(0,30);
    renderTape();
    if(S.positions[MINT]){ S.positions[MINT].priceSol=t.priceSol; S.positions[MINT].priceUsd=t.priceUsd; renderDeck(); }
  });
